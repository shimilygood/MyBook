...
myBookDirectives.directive('hello',function(){
	return {
		restrict:'AE',
		scope:{},      //独立scope
...
myBookDirectives.directive('drinks',function(){
	return {
		restrict:'AE',
		scope:{
			flavor:'@'
		},
		template:'<div><input type="text" ng-model="flavor"> {{flavor}}<br><button ng-click="flowers({name:flowers})">获取内容</button></div>',
		replace:true
		/*,
		link:function(scope,element,attrs){
			scope.flavor=attrs.flavor;
